...
import { ExampleJettonMaster, JettonMint, JettonBurn } from '../wrappers/ExampleJetton';
import { ExampleJettonWallet, JettonTransfer } from '../wrappers/ExampleJetton';
...
describe('JettonExample', () => {
    ...
    it('should Alice send 1 token to Bob', async () => {
        ...
        const bobJettonMint: JettonMint = {
            $$type: 'JettonMint',
            origin: bob.address,
        ...
        const bobMintyResult = await jettonMaster.send(
            bob.getSender(),
            {
                value: toNano('1'),
            },
        ...
        expect(bobBalanceAfter).toEqual(bobBalanceBefore + 1000000000n);
    });
    ...
    it('should Alice burn 1 token', async () => {
        ...
        const aliceBalanceAfter = (await aliceJettonContract.getGetWalletData()).balance;
        expect(aliceBalanceAfter).toEqual(aliceBalanceBefore - 1000000000n);
    });
});
